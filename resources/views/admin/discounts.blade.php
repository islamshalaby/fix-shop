...
@section('title' , __('messages.show_discounts'))
        ...
        <div class="statbox widget box box-shadow">
            ...
            <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <h4>{{ __('messages.show_discounts') }}</h4>
                </div>
            </div>
        </div>
                ...
                <table id="without-print" class="table table-hover non-hover" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th>{{ __('messages.min_products_count') }}</th>
                            <th>{{ __('messages.max_products_count') }}</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $discount)
                            <tr>
                                ...
                                <td>{{ $discount->min_products_number }}</td>
                                <td>{{ $discount->max_products_number }}</td>
                                @if(Auth::user()->update_data) 
                                    <td class="text-center blue-color" ><a href="{{ route('discounts.edit', $discount->id) }}" ><i class="far fa-edit"></i></a></td>
                                @endif
                                @if(Auth::user()->delete_data) 
                                    <td class="text-center blue-color" >
                                        <form action="{{ route('discounts.destroy', $discount->id) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <button type="submit" class="btn" onclick='return confirm("{{ __('messages.are_you_sure') }}");'><i class="far fa-trash-alt"></i></button>
                                        </form>
                                    </td>
                                @endif                                
                                <?php $i++; ?>
