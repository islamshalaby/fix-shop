...
class AppPagesController extends AdminController{
    ...
    // get privacy edit page
    public function privacy(){
        $data['setting'] = Setting::find(1);
        return view('admin.privacy' , ['data' => $data]);
    }
    ...
    public function PostPrivacy(Request $request){
        if(!$request->privacy_en){
            return redirect('admin-panel/app_pages/privacy')->with('status' , 'Privacy Policy Text in English Required');
        }
        if(!$request->privacy_ar){
            return redirect('admin-panel/app_pages/privacy')->with('status' , 'Privacy Policy Text in Arabic Required');
        }
        $setting = Setting::find(1);
        $setting->privacy_en = $request->privacy_en;
        $setting->privacy_ar = $request->privacy_ar;
        $setting->save();    
        return redirect('admin-panel/app_pages/privacy');
    }
